...
NumberRange 不是线程安全的<!--NumberRange是含有隐藏的不变性条件的，因为boolean isInRange(int i)是依赖于lower和upper两个变量，从某种意义来讲，属于把这两个变量发布了，最重要的是这两个变量彼此依赖-->，没有维持对下界和上界进行约束的不变性条件。假设取值范围在（0， 10），如果一个线程调用 setLower（5），而另一个线程调用 setUpper（4），那么在一些错误的执行时序中，比如下图所示
