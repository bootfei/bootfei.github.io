...
**ConcurrentHashMap不能被加锁 来执行独占访问，因此我们无法使用客户端加锁来创建新的原子操作。**<!--惨兮兮，无法使用组合的方式对ConcurrentHashMap进行自定义拓展了-->但是一些常见的复合操作，例如"若没有则添加"，"若相等则移除"，"若相等则替换"等，都已经实现为原子操作并且在ConcurrentMap接口中声明。
...
显然， 每当修改容器时都会复制底层数组， 这需要一定的开销， 特别是当容器的规模较大时。 **仅当迭代操作远远多于修改操作时， 才应该使用 “ 写入时复制” 容器**。 这个准则很好地描述了许多事件通知系统：在分发通知时需要迭代已注册监听器链表， 并调用每一个监听器， 在 大多数情况下， 注册和注销事件监听器的操作远少于接收事件通知的操作。<!--给我的感觉是，像InnoDB事务中的读锁和写锁，多个线程读的时候，是不会相互干扰的；写的时候，是会阻塞其他线程的-->
