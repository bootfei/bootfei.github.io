...
`Future` 表示一个任务的生命周期，并提供了相应的方法来判断是否已经完成或取消，以及「获取任务的结果」和「取消任务」等。 在 **程序清单 6-11** 中给出了 `Callable` 和 `Future`。<!--Callable与Future各自的作用一定要清楚，Callable是被提交给Executor的任务单元；Future是Executor获取Callable以后，Executor的返回结果-->
